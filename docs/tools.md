...
Fifteen MCP tools, four categories. Every tool returns a JSON object as a single text content block — that's the MCP convention YAPPING follows. Dates are ISO 8601 throughout (`2026-05-07T14:30:00Z` or `2026-05-07T10:30:00-07:00`).
...
List every calendar visible to Calendar.app on this Mac, with a `writable` flag that reflects whether YAPPING can mutate events on it (read-only subscribed calendars come back with `writable: false`).
