...
HAUNTED is local-only. There is no network, no remote service, no credential store. The threat surface is:
...
3. **Permission scope.** Calendar.app full access via the macOS Automation TCC grant gives HAUNTED full read/write to all calendars on the device. Users granting this access should be aware that HAUNTED will be able to read and modify any calendar event in any calendar (iCloud, On-My-Mac, Google synced into Apple Calendar, etc.).

## Supported versions

HAUNTED follows a rolling-release model. Only the **latest published version on npm** is supported for security fixes. Pin to `latest` in MCP client configs rather than an older version.

| Version   | Supported                                                     |
| --------- | ------------------------------------------------------------- |
| `0.2.x`   | ✅ (current)                                                  |
| `0.1.x`   | ⚠️ legacy `apple-calendar-mcp` name; migrate to `haunted-mcp` |
| `< 0.1.0` | ❌                                                            |
