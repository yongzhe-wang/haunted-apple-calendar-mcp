...
async function runCopyThenDelete(
  ...
  try {
    ...
    process.stderr.write(
      `[yapping-mcp] WARNING: copy-then-delete update created new event ${created.id} in "${canonicalTarget}", but failed to delete source event ${source.id} from "${source.calendar_name}": ${detail}. Duplicate event may exist; manual cleanup may be needed.\n`,
    );
  }
